...
import {IUpdateBranch, PropsUpdateBranch} from './interfaces';
...
function UpdateBranch({onClose}: PropsUpdateBranch) {
	const router = useRouter();
	const queryClient = useQueryClient();

	const {_uuidBranches} = router.query;

	const [form, setForm] = useState<IUpdateBranch>({
		name: '',
		note: '',
		matp: '',
		maqh: '',
	...
	useQuery([QUERY_KEY.detail_branches, _uuidBranches], {
		queryFn: () =>
			httpRequest({
				http: branchesServices.detailBranches({
					uuid: _uuidBranches as string,
				}),
			}),
		onSuccess(data) {
			if (data) {
	...
	const funcUpdateBranch = useMutation({
		mutationFn: () => {
			return httpRequest({
				showMessageFailed: true,
				showMessageSuccess: true,
				msgSuccess: 'Cập nhật chi nhánh thành công!',
	...
	return (
		<Form form={form} setForm={setForm} onSubmit={funcUpdateBranch.mutate}>
			<Loading loading={funcUpdateBranch.isLoading} />
			<div className={styles.container}>
				<h4 className={styles.title}>Chỉnh sửa chi nhánh</h4>
